...
public class MainFileController {
	...
	public boolean readMainFile(String file) {
		...
		for (LineCode line : mainFile.getLineCodeList()) {
			...
			if (dictionaryController.assignValueToWord(code)) {
				line.setValidate(true);
			}
			if (dictionaryController.getValueOfProduct(code)) {
				line.setValidate(true);
			}
			if (dictionaryController.conversionQuery(code)) {
				line.setValidate(true);
			}
			if (dictionaryController.productQuery(code)) {
				line.setValidate(true);
			}
		}
		return true;
